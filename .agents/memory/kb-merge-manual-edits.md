...
**Trap 3 — Apply button gating on dirtiness:** the modal's Apply button can't be gated on `report.collapsed > 0` alone. A rename-only or split-out edit recomputes a report with `collapsed === 0` yet still must be appliable (otherwise the member's edit is silently lost). Track a `dirty` flag set on any successful `callEdit` and show Apply on `collapsed > 0 || selected.size > 0 || dirty`. Relatedly, the live-preview debounce must bump its sequence ref when the selection clears (`effectiveVariants.length === 0`), or an in-flight preview response repopulates a stale combined-sales count after clear.
