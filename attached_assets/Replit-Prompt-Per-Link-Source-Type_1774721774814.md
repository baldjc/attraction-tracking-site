...
When creating a new tracking link inside a campaign (the "+ New Link" flow), add a **source picker** — a dropdown or selectable button group with options:
- YouTube
- LinkedIn
- Instagram
- Email
...
Default to "YouTube" since that's the most common. The source picker should also appear when editing an existing link.
...
### 4. Campaign Creation Modal

Remove the campaign type selector from the campaign creation flow. Campaigns no longer have a type. The source is chosen per-link.
