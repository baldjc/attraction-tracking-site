...
**Dev stale-cache recovery (`npm run dev:reset`):** After a task merge / post-merge reconciliation, the Turbopack `.next/dev` cache can go stale and make a cluster of `/api/**` route handlers return **404** (they fall through to Next's built-in `/_not-found`) even though pages and the `/api/auth/[...nextauth]` catch-all still work — and a plain restart that reuses `.next` only regresses it further. A 404 cluster like this is **never** a migration/schema issue (that would be a 500). Fix: `npm run dev:reset` (kills port 5000, `rm -rf .next`, restarts clean). See `.agents/memory/turbopack-dev-stale-cache-404.md`.
