...
export default function ArcScriptBuilderPage() {
  ...
  async function callAPI(step: string, extraData = {}) {
    setLoading(true);
    const allStepData = {
    ...
    const res = await fetch("/api/ai-tools/arc-script-builder", {
      ...
      body: JSON.stringify({ step, allStepData }),
    });
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-3">
        <Link href="/member/ai-tools" className="p-1.5 rounded-lg hover:bg-[#1e2a38]/10 transition-colors">
        ...
        <div className="bg-white border border-[#1e2a38]/10 rounded-2xl p-6 space-y-5">
          <div>
            ...
            {avatar?.avatarName ? (
              ...
              <p className="text-sm text-amber-600">No avatar saved — <Link href="/member/ai-tools/avatar-architect" className="underline">build one first</Link></p>
            )}
          </div>
        ...
        <div className="bg-white border border-[#1e2a38]/10 rounded-2xl p-6 space-y-5">
          <div>
            ...
            <p className="text-sm text-[#1e2a38]/50">
              Don't have a title yet? <Link href="/member/ai-tools/title-creator" className="text-[#3dc3ff] underline" target="_blank">Open Title Creator →</Link>
            </p>
          </div>
      ...
      {step === 5 && scriptOutline && (
        <div className="space-y-5">
          ...
          {(scriptOutline.insights || []).map((ins, i) => (
            <div key={i} className="bg-white border border-[#1e2a38]/10 rounded-2xl p-5">
              <p className="text-xs font-semibold text-[#1e2a38]/40 uppercase tracking-wide mb-3">Insight {i + 1} — Value Loop</p>
              <div className="space-y-2">
                {[["What", ins.what], ["Why", ins.why], ["When", ins.when], ["Story Proof", ins.story], ["What This Means", ins.connection]].map(([label, text]) => (
                  <div key={label}>
                    <span className="text-xs font-semibold text-[#3dc3ff]">{label}: </span>
                    <span className="text-sm text-[#1e2a38]">{text}</span>
                  </div>
                ))}
                {ins.curiosity_bridge && (
                  <div className="mt-2 pt-2 border-t border-[#1e2a38]/5">
                    <span className="text-xs font-semibold text-[#1e2a38]/40">Bridge: </span>
                    <span className="text-sm text-[#1e2a38]/60 italic">{ins.curiosity_bridge}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {[
            ...
            { label: "Closing + Lead Magnet #3", content: scriptOutline.closing },
          ].map((section) => (
            <div key={section.label} className="bg-white border border-[#1e2a38]/10 rounded-2xl p-5">
              <p className="text-xs font-semibold text-[#1e2a38]/40 uppercase tracking-wide mb-3">{section.label}</p>
              <p className="text-sm text-[#1e2a38] leading-relaxed whitespace-pre-wrap">{section.content}</p>
          ...
          <div className="flex gap-3 pb-6">
            <Link
              href="/member/script-review"
              className="flex-1 border border-[#3dc3ff] text-[#3dc3ff] py-3 rounded-xl font-semibold hover:bg-[#3dc3ff]/5 transition-colors text-center"
            >
              Review with Script Review →
            </Link>
            <button
              onClick={() => { setStep(0); setSaved(false); setScriptOutline(null); setChecklist(null); setOpeningOptions([]); setSelectedOpening(""); }}
              className="flex-1 border border-[#1e2a38]/20 text-[#1e2a38] py-3 rounded-xl font-semibold hover:bg-[#1e2a38]/5 transition-colors"
            >
              Start New Script
            </button>
          </div>
