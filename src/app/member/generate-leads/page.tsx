...
function TrainingTab({ sections, loading, pct, onSwitchTab }: {
  ...
  if (loading) {
    return (
      <div className="animate-pulse space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-24 bg-[var(--abv-border-strong)] dark:bg-white/10 rounded-xl" />
        ))}
      </div>
    );
  }
  ...
  if (sections.length === 0) {
    return (
      <div className="bg-white dark:bg-[#1a1a1a] border border-[var(--abv-text)]/10 dark:border-white/10 rounded-lg p-12 text-center">
        ...
        <h2 className="font-semibold text-[var(--abv-text)] dark:text-white mb-2">Training modules are on the way</h2>
  ...
  return (
    <div className="space-y-4">
      {sections.map((section) => {
        ...
        return (
          <Link
            ...
            className="group bg-white dark:bg-[#1a1a1a] border border-[var(--abv-text)]/10 dark:border-white/10 rounded-xl p-5 flex items-start gap-4 hover:border-[var(--abv-azure)]/40 hover:shadow-sm transition-all"
          >
            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
              complete
                ...
                : sectionPct > 0
                ? "bg-[var(--abv-dark)]/10"
                : "bg-[var(--abv-bg)] dark:bg-white/5"
            }`}>
              {complete ? (
                <CheckCircleIcon className="w-5 h-5 text-green-600 dark:text-green-400" />
              ) : (
                <span className="text-sm font-bold text-[var(--abv-text)]/40 dark:text-white/40">
                  {section.lessonCount}
                </span>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-semibold text-[var(--abv-text)] dark:text-white group-hover:text-[var(--abv-azure)] transition-colors">
                    {section.title}
                  </h3>
                  {section.description && (
                    <p className="text-sm text-[var(--abv-text)]/50 dark:text-white/50 mt-0.5 line-clamp-2">
                      {section.description}
                    </p>
                  )}
                </div>
                <span className="text-xs font-medium text-[var(--abv-text)]/40 dark:text-white/40 shrink-0 mt-0.5">
                  {section.completedCount}/{section.lessonCount} lessons
                </span>
              </div>
              {section.lessonCount > 0 && (
                <div className="mt-3 h-1.5 bg-[var(--abv-border-strong)] dark:bg-white/10 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${complete ? "bg-green-500" : "bg-[var(--abv-dark)]"}`}
      ...
      {pct === 100 && (
        <div className="bg-[var(--abv-dark)]/5 dark:bg-[var(--abv-dark)]/10 border border-[var(--abv-azure)]/20 rounded-xl p-6 text-center">
          <div className="text-3xl mb-2">🎉</div>
          <h3 className="font-semibold text-[var(--abv-text)] dark:text-white mb-1">Training Complete!</h3>
...
function GenerateLeadsPageInner() {
  ...
  return (
    <div>
      ...
      {showHeroCard && !hasCampaigns && (
        pct === 0 ? (
          <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-200 dark:border-[#2a2a2a] p-6 mb-6">
            <h2 className="text-lg font-bold text-[var(--abv-text)] dark:text-[#e2e8f0] mb-2">Welcome to Generate Leads</h2>
            <p className="text-sm text-[var(--abv-text)]/60 dark:text-[#94a3b8] mb-5">
          ...
          <div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/30 rounded-xl p-6 mb-6 text-center">
            <div className="text-3xl mb-2">🎉</div>
            <h3 className="font-semibold text-[var(--abv-text)] dark:text-[#e2e8f0] mb-1">Training Complete!</h3>
            <p className="text-sm text-[var(--abv-text)]/60 dark:text-[#94a3b8] mb-4">
          ...
          <div className="bg-[var(--abv-dark)]/5 dark:bg-[var(--abv-dark)]/10 border border-[var(--abv-azure)]/20 rounded-lg px-5 py-3 mb-6 flex items-center gap-4">
            <div className="flex-1 min-w-0">
              <span className="text-sm font-medium text-[var(--abv-text)] dark:text-[#e2e8f0]">
      ...
      {/* Tab bar */}
      <div className="flex gap-1 border-b border-[var(--abv-text)]/10 dark:border-white/10 mb-6">
...
export default function GenerateLeadsPage() {
  return (
    <Suspense fallback={
      <div className="animate-pulse space-y-4">
        <div className="h-8 bg-[var(--abv-border-strong)] dark:bg-white/10 rounded w-1/3" />
        <div className="h-4 bg-[var(--abv-border-strong)] dark:bg-white/10 rounded w-1/2" />
      </div>
    }>
