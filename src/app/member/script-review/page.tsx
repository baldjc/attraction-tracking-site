...
export default function ScriptReviewPage() {
  ...
  function ResultDisplay({ r }: { r: ReviewResult }) {
        ...
        <div className={`rounded-xl p-6 text-center border ${scoreBgBlock(overallScore)}`}>
          ...
          {delta != null && baselineAvg != null && (
            <p className="text-sm mt-2 text-[#1e2a38]/60">
              Your baseline: <span className="font-semibold">{baselineAvg.toFixed(1)}</span>
              {" — this script scores "}
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-[#1e2a38]/60 mt-1">
          Paste your script or transcript below to get scored against the 16 Attraction principles before you publish.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
        ...
        {loading && (
          <p className="text-xs text-center text-[#1e2a38]/40">This usually takes 20–40 seconds…</p>
        )}
      </form>
        ...
        <div id="results" className="space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-[#1e2a38]">
              {viewedResult ? `Review: "${viewedResult.videoTitle}"` : "Your Results"}
            </h2>
            {result && !viewedResult && (
