...
a) GAP CHECK — Flag if any of these are missing:
   ...
   - If data is sparse, proceed automatically. Do NOT block the build. Note to the member that data placeholders will be inserted into the final script for them to fill in before filming, then continue. Placeholders in the final script must follow the format in requirement 17 — specific about what stat to find, where to find it, and the editorial reaction to deliver on camera. The member can swap real numbers in after.
...
IMPORTANT: Do NOT re-state the creator's expertise or credentials in the closing. No "after helping X families…", no track-record brags, no "I can say this because…" framing. Credibility was already layered in via the expertise bridge in the opening — the close is purely curiosity + the next hook. Drop any credential sentence from the closing entirely.
...
Model closing (flows right out of last insight, no credential brag): "...and that's what separates the families who love their next home from the ones who regret it two years later. I put together a free guide called [lead magnet] that walks you through exactly this — link's in the description. Now, most {{MEMBER_CITY}} homebuyers obsess over interest rates and wait for the 'perfect' moment. But the ones who regret their purchase weren't wrong about the market — they were wrong about something way more important. That's exactly what I break down in this next video right here."
