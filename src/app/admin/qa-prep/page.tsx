  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#1e2a38]">Q&amp;A Call Prep</h1>
          <p className="text-sm text-[#1e2a38]/60 mt-1">{fmtThursday(nextThursday)}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={load}
            disabled={loading}
            className="inline-flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 text-[#1e2a38]/70 transition-colors disabled:opacity-40"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg bg-[#1e2a38] hover:bg-[#1e2a38]/90 text-white transition-colors disabled:opacity-40"
          >
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm text-[#1e2a38]/60">Prep for week of:</label>
        <input
          type="date"
          value={selectedDate}
          onChange={(e) => setSelectedDate(e.target.value)}
          className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 text-[#1e2a38] focus:outline-none focus:ring-2 focus:ring-[#3dc3ff]/50"
        />
        {selectedDate && (
          <button
            onClick={() => setSelectedDate("")}
            className="text-xs text-[#1e2a38]/40 hover:text-[#1e2a38]"
          >
            Reset to this week
          </button>
        )}
      ...
      {loading && (
        <div className="flex items-center justify-center py-16 text-[#1e2a38]/40">
      ...
      {!loading && data && (
        <>
                ...
                {data.celebrate.map((m: any) => (
                  <div key={m.userId} className="bg-white rounded-lg p-4 border border-green-200">
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <Link href={`/admin/members/${m.userId}`} className="font-semibold text-[#1e2a38] hover:text-[#3dc3ff] text-sm">
                        {m.name}
                      </Link>
          ...
          <div className="bg-[#ffe5ea] border border-[#ff0033]/20 rounded-xl p-6">
            ...
            <div className="space-y-3">
              {data.commonGaps.map((gap: any, i: number) => (
                <div key={i} className="bg-white rounded-lg p-4 border border-[#ff0033]/10 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-[#1e2a38]">{gap.principle}</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <h2 className="text-base font-semibold text-[#1e2a38] mb-4">📋 Per-Member Notes</h2>
              ...
              {data.perMember.map((m: any) => {
                ...
                return (
                  <div key={m.userId} className="border border-gray-100 rounded-xl overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="font-medium text-sm text-[#1e2a38]">{m.name}</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[#1e2a38]/30 text-xs">{isExpanded ? "▲" : "▼"}</span>
                      </div>
                    </button>
                      ...
                      <div className="px-4 pb-4 space-y-3 border-t border-gray-100">
                        <div className="flex items-center gap-3 pt-3">
                          ...
                          <Link
                            ...
                            className="inline-flex items-center gap-1 text-xs text-[#3dc3ff] hover:underline"
                          >
                            View report
                          ...
                          <div>
                            ...
                            <div className="space-y-1">
                              {m.improvements.slice(0, 3).map((imp: any, i: number) => (
                                <p key={i} className="text-xs text-[#1e2a38]/70">
                        ...
                        {m.qaFlags.length > 0 && (
                          <div>
                            <p className="text-xs font-semibold text-[#3dc3ff] mb-1">If they're on the call:</p>
                            <div className="space-y-1">
