...
export default function RunReportPage({ params }: { params: Promise<{ runId: string }> }) {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto">
        ...
        <div className="h-64 bg-white border border-[var(--abv-text)]/10 rounded-xl animate-pulse" />
      </div>
    );
  }
  ...
  if (!run) {
    return (
      <div className="max-w-4xl mx-auto">
        <Link href="/admin/intelligence/runs" className="text-sm text-[var(--abv-text)]/50 hover:text-[var(--abv-text)]">← Runs</Link>
        <p className="mt-6 text-[var(--abv-text)]/50">Run not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <Link href="/admin/intelligence/runs" className="text-sm text-[var(--abv-text)]/50 hover:text-[var(--abv-text)]">← All Runs</Link>
          <h1 className="text-xl font-bold text-[var(--abv-text)] mt-1 break-all">{run.inputChannelUrl}</h1>
          {run.client && (
            <p className="text-sm text-[var(--abv-text)]/60 mt-0.5">{run.client.name} · {run.client.city}</p>
          )}
          <p className="text-xs text-[var(--abv-text)]/30 mt-0.5">
      ...
      {run.status === "RUNNING" || run.status === "PENDING" ? (
        <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-10 text-center">
          <div className="w-12 h-12 rounded-full border-4 border-[var(--abv-azure)] border-t-transparent animate-spin mx-auto mb-4" />
          <p className="font-semibold text-[var(--abv-text)]">
        ...
        <>
          {reportJson && (
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[var(--abv-text)]">{reportJson.videoCount ?? "—"}</p>
                <p className="text-xs text-[var(--abv-text)]/50 mt-0.5">Videos Analysed</p>
              </div>
              <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-green-600">{reportJson.outlierCount ?? outliers.length}</p>
                <p className="text-xs text-[var(--abv-text)]/50 mt-0.5">Outliers Found</p>
              </div>
              <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-[var(--abv-azure)]">{analyses.length}</p>
                <p className="text-xs text-[var(--abv-text)]/50 mt-0.5">Deep Analyses</p>
              </div>
            </div>
          )}

          <div className="flex gap-2 mb-4">
            <button onClick={() => setTab("report")} className={`px-4 py-2 text-sm font-semibold rounded-lg border transition-colors ${tab === "report" ? "bg-[var(--abv-text)] text-white border-[var(--abv-text)]" : "bg-white text-[var(--abv-text)]/60 border-[var(--abv-text)]/15 hover:border-[var(--abv-text)]/30"}`}>
              📄 Full Report
            </button>
            {outliers.length > 0 && (
              <button onClick={() => setTab("outliers")} className={`px-4 py-2 text-sm font-semibold rounded-lg border transition-colors ${tab === "outliers" ? "bg-[var(--abv-text)] text-white border-[var(--abv-text)]" : "bg-white text-[var(--abv-text)]/60 border-[var(--abv-text)]/15 hover:border-[var(--abv-text)]/30"}`}>
          ...
          {tab === "report" && run.reportMarkdown && (
            <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-6">
              <pre className="whitespace-pre-wrap text-sm text-[var(--abv-text)] font-sans leading-relaxed">{run.reportMarkdown}</pre>
            </div>
          )}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {analysis && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {analysis.analysis?.hookType && <span className="px-2 py-0.5 bg-[var(--abv-dark)]/10 text-[var(--abv-azure)] text-xs rounded-full">{analysis.analysis.hookType}</span>}
                        ...
                        {analysis?.analysis?.whyItWorked && (
                          <p className="text-xs text-[var(--abv-text)]/50 mt-1.5 italic line-clamp-2">{analysis.analysis.whyItWorked}</p>
                        )}
                      </div>
                      <button
                        onClick={() => saveOutlierToSwipe(ov)}
                        disabled={savingSwipe === ov.ytVideoId || isSwiped}
                        className={`px-3 py-1.5 text-xs font-semibold rounded-lg border shrink-0 transition-colors ${isSwiped ? "bg-green-50 text-green-700 border-green-200" : "bg-[var(--abv-bg)] text-[var(--abv-text)]/60 border-[var(--abv-text)]/10 hover:text-[var(--abv-text)]"} disabled:opacity-50`}
                      >
