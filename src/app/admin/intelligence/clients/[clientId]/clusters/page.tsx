    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-[var(--abv-text)] mt-1">Topic Clusters</h1>
        </div>
      ...
      {ideaMsg && (
        <div className="mb-4 bg-[var(--abv-dark)]/10 border border-[var(--abv-azure)]/30 rounded-lg px-4 py-3 text-sm text-[var(--abv-text)]">
          {ideaMsg} <Link href={`/admin/intelligence/clients/${clientId}/ideas`} className="text-[var(--abv-azure)] underline ml-1">View Ideas →</Link>
        </div>
      )}

      {showAdd && (
        <form onSubmit={handleAdd} className="bg-white border border-[var(--abv-azure)]/30 rounded-xl p-5 mb-5 space-y-3">
          <p className="text-sm font-semibold text-[var(--abv-text)]">New Topic Cluster</p>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-10 text-center">
          ...
          <p className="font-semibold text-[var(--abv-text)]">No clusters yet</p>
          <p className="text-sm text-[var(--abv-text)]/50 mt-1">Group keywords into topic clusters, then generate content ideas from them.</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {clusters.map((cluster) => (
            <div key={cluster.id} className="bg-white border border-[var(--abv-text)]/10 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 bg-[var(--abv-bg)] border-b border-[var(--abv-text)]/8">
                <div>
                  <p className="text-sm font-semibold text-[var(--abv-text)]">{cluster.name}</p>
                  {cluster.theme && <p className="text-xs text-[var(--abv-text)]/50">{cluster.theme}</p>}
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-[var(--abv-text)]/40">{cluster.keywords.length} keywords</span>
              ...
              {cluster.keywords.length > 0 && (
                <div className="px-5 py-3 flex flex-wrap gap-1.5">
                  {cluster.keywords.map((kw) => (
                    <span key={kw.id} className="px-2.5 py-1 bg-[var(--abv-bg)] text-xs text-[var(--abv-text)]/70 rounded-full border border-[var(--abv-text)]/8">
                      {kw.keyword}
                      {kw.volume != null && <span className="ml-1 text-[var(--abv-text)]/40">{kw.volume.toLocaleString()}</span>}
                    </span>
                  ))}
                </div>
              )}
              {cluster.notes && <p className="px-5 pb-3 text-xs text-[var(--abv-text)]/50">{cluster.notes}</p>}
            </div>
          ))}
        </div>
      )}
