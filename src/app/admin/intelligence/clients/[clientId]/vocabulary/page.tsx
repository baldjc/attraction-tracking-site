    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-[var(--abv-text)] mt-1">Vocabulary Profile</h1>
          <p className="text-sm text-[var(--abv-text)]/50 mt-0.5">Terms, phrases, and language patterns specific to this client&apos;s market.</p>
        </div>
      ...
      {showAdd && (
        <form onSubmit={handleAdd} className="bg-white border border-[var(--abv-azure)]/30 rounded-xl p-5 mb-5 space-y-3">
          <p className="text-sm font-semibold text-[var(--abv-text)]">New Vocabulary Term</p>
          <div className="grid grid-cols-2 gap-3">
            <input type="text" value={term} onChange={(e) => setTerm(e.target.value)} placeholder="Term *" required
              className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--abv-azure)]/40" />
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={saving} className="px-4 py-2 bg-[var(--abv-dark)] text-white text-sm font-semibold rounded-lg disabled:opacity-50">{saving ? "Saving…" : "Save Term"}</button>
      ...
      <div className="mb-4">
        <input
          ...
          className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--abv-azure)]/40 bg-white"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-10 text-center">
          ...
          <p className="font-semibold text-[var(--abv-text)]">No vocabulary terms yet</p>
          <p className="text-sm text-[var(--abv-text)]/50 mt-1">Add market-specific terms, neighbourhood names, buyer personas, and phrases that resonate with this client&apos;s audience.</p>
        </div>
      ) : (
        <div>
          {categories.length > 0 && (
            <div className="mb-4 flex flex-wrap gap-1.5">
              {categories.map((cat) => (
                <span key={cat} className="px-2.5 py-1 bg-[var(--abv-bg)] text-xs text-[var(--abv-text)]/60 rounded-full border border-[var(--abv-text)]/8">{cat}</span>
              ))}
            </div>
          )}
          <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl divide-y divide-[var(--abv-text)]/6">
            {filtered.map((v) => (
              <div key={v.id} className="p-4 hover:bg-[var(--abv-bg)]/40 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-[var(--abv-text)] text-sm">{v.term}</p>
                      {v.category && <span className="px-2 py-0.5 bg-[var(--abv-bg)] text-xs text-[var(--abv-text)]/50 rounded-full">{v.category}</span>}
                    </div>
                    {v.definition && <p className="text-sm text-[var(--abv-text)]/60 mt-0.5">{v.definition}</p>}
                    {v.exampleUsage && (
                      <p className="text-xs text-[var(--abv-text)]/40 mt-1 italic">&ldquo;{v.exampleUsage}&rdquo;</p>
                    )}
                  </div>
