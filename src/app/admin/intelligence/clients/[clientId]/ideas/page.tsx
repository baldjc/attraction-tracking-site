    ...
    <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-[var(--abv-text)] mt-1">Content Ideas</h1>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setShowManual((v) => !v)} className="px-4 py-2 bg-[var(--abv-bg)] text-[var(--abv-text)]/70 text-sm font-semibold rounded-lg border border-[var(--abv-text)]/10 hover:bg-[#eee] transition-colors">
            + Manual Idea
          </button>
      ...
      {genMsg && (
        <div className="mb-4 bg-[var(--abv-dark)]/10 border border-[var(--abv-azure)]/30 rounded-lg px-4 py-3 text-sm text-[var(--abv-text)]">{genMsg}</div>
      )}

      {showManual && (
        <form onSubmit={saveManual} className="bg-white border border-[var(--abv-azure)]/30 rounded-xl p-5 mb-5 space-y-3">
          <p className="text-sm font-semibold text-[var(--abv-text)]">New Manual Idea</p>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={saving} className="px-4 py-2 bg-[var(--abv-dark)] text-white text-sm font-semibold rounded-lg disabled:opacity-50">{saving ? "Saving…" : "Save Idea"}</button>
            <button type="button" onClick={() => setShowManual(false)} className="px-4 py-2 text-sm text-[var(--abv-text)]/50">Cancel</button>
          </div>
        </form>
      )}
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {["all", "idea", "scripting", "filming", "editing", "published"].map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-3 py-1.5 text-xs font-semibold rounded-full border transition-colors ${filter === s ? "bg-[var(--abv-text)] text-white border-[var(--abv-text)]" : "bg-white text-[var(--abv-text)]/60 border-[var(--abv-text)]/15 hover:border-[var(--abv-text)]/30"}`}
          >
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl p-10 text-center">
          ...
          <p className="font-semibold text-[var(--abv-text)]">{filter === "all" ? "No ideas yet" : `No ideas with status "${filter}"`}</p>
          <p className="text-sm text-[var(--abv-text)]/50 mt-1">Generate ideas from your topic clusters or add them manually.</p>
        </div>
      ) : (
        <div className="bg-white border border-[var(--abv-text)]/10 rounded-xl divide-y divide-[var(--abv-text)]/6">
          {filtered.map((idea) => (
            <div key={idea.id} className="p-4 hover:bg-[var(--abv-bg)]/40 transition-colors">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-[var(--abv-text)] text-sm leading-snug">{idea.title}</p>
                  {idea.outline && <p className="text-xs text-[var(--abv-text)]/50 mt-1 line-clamp-2">{idea.outline}</p>}
                  <div className="flex gap-2 mt-2 flex-wrap">
                    ...
                    <span className="text-xs text-[var(--abv-text)]/30">{new Date(idea.createdAt).toLocaleDateString("en-CA")}</span>
                  </div>
                </div>
