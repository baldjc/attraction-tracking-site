  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#1e2a38]">Course Lessons</h1>
          <p className="text-sm text-[#1e2a38]/50 mt-1">Manage the Attraction by Video course content and transcripts</p>
        </div>
        <button onClick={openAdd} className="flex items-center gap-2 bg-[#3dc3ff] text-white px-4 py-2.5 rounded-xl text-sm font-semibold hover:bg-[#3dc3ff]/90 transition-colors">
          <PlusIcon className="w-4 h-4" /> Add Lesson
        </button>
      </div>

      {loading ? (
        <div className="text-center py-16 text-[#1e2a38]/40">Loading lessons...</div>
      ) : (
          ...
          {sessionGroups.map(({ session, lessons: sLessons }) => (
            <div key={session} className={CARD}>
              <div className="px-6 py-4 border-b border-[#1e2a38]/10">
                <h2 className="font-semibold text-[#1e2a38]">Session {session}</h2>
                <p className="text-xs text-[#1e2a38]/40 mt-0.5">{sLessons.length} lesson{sLessons.length !== 1 ? "s" : ""}</p>
              </div>
              {sLessons.length === 0 ? (
                <div className="px-6 py-8 text-center text-sm text-[#1e2a38]/30">No lessons in this session</div>
              ) : (
                <div className="divide-y divide-[#1e2a38]/5">
                  {sLessons.map((lesson) => (
                    <div key={lesson.id}>
                      <div className="px-6 py-4 flex items-start gap-4">
                        <div className="flex-shrink-0 w-10 h-10 bg-[#1e2a38]/5 rounded-xl flex items-center justify-center">
                          <span className="text-xs font-bold text-[#1e2a38]/60">{lesson.lessonNumber}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1.5">
                            <span className="font-semibold text-[#1e2a38] text-sm">{lesson.title}</span>
                            {lesson.skoolUrl && (
                              <a href={lesson.skoolUrl} target="_blank" rel="noopener noreferrer" className="text-[10px] text-[#3dc3ff] hover:underline">Skool ↗</a>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-3 text-xs text-[#1e2a38]/40">
                            ...
                            {processResult?.id === lesson.id && (
                              <span className="text-[#3dc3ff] flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          {lesson.fullTranscript && (
                            <button
                              ...
                              className="p-2 rounded-lg text-[#1e2a38]/30 hover:text-[#3dc3ff] hover:bg-[#3dc3ff]/5 transition-colors disabled:opacity-40"
                            >
                              <ArrowPathIcon className={`w-4 h-4 ${processingId === lesson.id ? "animate-spin" : ""}`} />
                            </button>
                          )}
                          <button onClick={() => openEdit(lesson)} className="p-2 rounded-lg text-[#1e2a38]/30 hover:text-[#3dc3ff] hover:bg-[#3dc3ff]/5 transition-colors">
                            <PencilIcon className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteConfirm(lesson.id)} className="p-2 rounded-lg text-[#1e2a38]/30 hover:text-red-500 hover:bg-red-50 transition-colors">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl border border-[#1e2a38]/10 shadow-xl w-full max-w-2xl my-8">
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#1e2a38]/10">
              <h2 className="font-bold text-[#1e2a38] text-lg">{editingId ? "Edit Lesson" : "Add Lesson"}</h2>
              <button onClick={() => setShowModal(false)} className="text-[#1e2a38]/40 hover:text-[#1e2a38] text-xl">✕</button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-[#1e2a38] mb-1.5">Lesson Number</label>
                ...
                <div>
                  <label className="block text-sm font-semibold text-[#1e2a38] mb-1.5">Session</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1e2a38] mb-1.5">Lesson Title</label>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1e2a38] mb-1.5">Skool URL <span className="font-normal text-[#1e2a38]/40">(optional)</span></label>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1e2a38] mb-2">Attraction Principles</label>
                    ...
                    <button
                      ...
                      className={`text-xs font-medium px-2.5 py-1 rounded-full border transition-all ${
                        form.principles.includes(p)
                          ...
                          : "bg-white text-[#1e2a38]/50 border-[#1e2a38]/20 hover:border-[#1e2a38]/40"
                      }`}
                    >
                      {p}
                    </button>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1e2a38] mb-1.5">
                  Transcript <span className="font-normal text-[#1e2a38]/40">(paste full transcript — Claude will segment it)</span>
                </label>
                <textarea
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={saveLesson} disabled={saving} className="flex-1 bg-[#3dc3ff] text-white py-2.5 rounded-xl font-semibold text-sm disabled:opacity-50 hover:bg-[#3dc3ff]/90 transition-colors">
                  {saving ? "Saving..." : editingId ? "Save Changes" : "Create Lesson"}
                </button>
                {editingId && form.fullTranscript && (
                  <button onClick={reprocessFromModal} disabled={saving} className="flex items-center gap-2 px-4 py-2.5 border border-[#1e2a38]/20 rounded-xl text-sm text-[#1e2a38]/60 hover:bg-gray-50 disabled:opacity-50 transition-colors">
                    <ArrowPathIcon className="w-4 h-4" /> Re-process
                  </button>
                )}
                <button onClick={() => setShowModal(false)} className="px-4 py-2.5 border border-[#1e2a38]/20 rounded-xl text-sm text-[#1e2a38]/60 hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
              </div>
            </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            <h3 className="font-bold text-[#1e2a38] mb-2">Delete Lesson?</h3>
            <p className="text-sm text-[#1e2a38]/60 mb-5">This will also delete all knowledge base segments for this lesson. This cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => deleteLesson(deleteConfirm)} className="flex-1 bg-red-500 text-white py-2.5 rounded-xl font-semibold text-sm hover:bg-red-600 transition-colors">Delete</button>
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 border border-[#1e2a38]/20 rounded-xl text-sm text-[#1e2a38]/60 hover:bg-gray-50 transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
...
function SegmentList({ lessonId }: { lessonId: string }) {
  ...
  if (loading) return <div className="px-6 py-3 text-xs text-[#1e2a38]/40">Loading segments...</div>;
  if (!segments.length) return <div className="px-6 py-3 text-xs text-[#1e2a38]/40">No segments yet</div>;
  ...
  return (
    <div className="bg-[#1e2a38]/2 border-t border-[#1e2a38]/5 divide-y divide-[#1e2a38]/5">
      {segments.map((seg, i) => (
        <div key={seg.id} className="px-6 py-3 flex gap-3">
          <span className="text-xs text-[#1e2a38]/30 w-5 flex-shrink-0 mt-0.5">{i + 1}</span>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-semibold text-[#1e2a38]">{seg.subTopic}</span>
              {(seg.timestampStart != null) && (
                <span className="text-[10px] text-[#1e2a38]/30">{fmtTime(seg.timestampStart)}–{fmtTime(seg.timestampEnd)}</span>
              )}
            </div>
            <p className="text-xs text-[#1e2a38]/60 mb-1">{seg.summary}</p>
