      ...
      <div className="hidden lg:flex flex-col w-64 shrink-0 border-r border-gray-200 dark:border-[#2a2a2a] bg-white dark:bg-[#1a1a1a]">
        ...
        <div className="flex-1 overflow-y-auto">
          {membersLoading ? (
            ...
            <p className="text-xs text-[#2f3437]/40 dark:text-white/30 text-center py-6">No members found</p>
          ) : (
        ...
        <div className="lg:hidden p-4 border-b border-gray-200 dark:border-[#2a2a2a] bg-white dark:bg-[#1a1a1a] shrink-0">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 text-sm border border-gray-200 dark:border-[#2a2a2a] rounded-lg px-3 py-2 bg-white dark:bg-[#1a1a1a] hover:border-[#6ba3c7] transition-colors w-full disabled:opacity-50"
            >
              <CalendarDaysIcon className="w-4 h-4 text-[#6ba3c7] shrink-0" />
              <span className={`flex-1 text-left truncate ${selected ? "text-[#2f3437] dark:text-[#e2e8f0]" : "text-[#2f3437]/40 dark:text-white/30"}`}>
              ...
              <div className="absolute top-full left-0 mt-1 w-full bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#2a2a2a] rounded-xl shadow-lg z-30 overflow-hidden">
                <div className="p-2 border-b border-gray-100 dark:border-[#2a2a2a]">
                  <input
                    ...
                    autoFocus
                    placeholder="Search members…"
                    className="w-full text-sm px-3 py-1.5 border border-gray-200 dark:border-[#2a2a2a] rounded-lg bg-white dark:bg-[#1a1a1a] text-[#2f3437] dark:text-[#e2e8f0] focus:outline-none focus:ring-2 focus:ring-[#6ba3c7]/30"
                  />
                </div>
                <div className="max-h-60 overflow-y-auto">
                  {mobileFiltered.map((m) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors ${selected?.id === m.id ? "bg-[#6ba3c7]/5 dark:bg-[#6ba3c7]/10" : ""}`}
                    >
                      <p className="text-sm font-medium text-[#2f3437] dark:text-[#e2e8f0] truncate">{m.fullName ?? m.email}</p>
                      {m.fullName && <p className="text-xs text-[#2f3437]/50 dark:text-white/30 truncate">{m.email}</p>}
                    </button>
                  ))}
                </div>
              </div>
          ...
          {tierLoading ? (
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <CalendarDaysIcon className="w-10 h-10 text-[#2f3437]/15 dark:text-white/10 mx-auto mb-3" />
                <p className="text-sm text-[#2f3437]/40 dark:text-white/30">
