  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#2f3437]/60 mt-1">View saved ARC scripts for any member.</p>
      </div>

      <div className="bg-white border border-[#2f3437]/10 rounded-xl px-5 py-4 mb-5">
        <label className="block text-sm font-semibold text-[#2f3437] mb-2 flex items-center gap-2">
          <UserIcon className="w-4 h-4 text-[#6ba3c7]" />
          Select a member
        </label>
        {membersLoading ? (
          <div className="h-10 bg-[#f7f6f3] rounded-lg animate-pulse" />
        ) : (
          <select
            value={selectedMemberId}
            onChange={(e) => setSelectedMemberId(e.target.value)}
            className="w-full bg-[#f7f6f3] border border-[#2f3437]/15 rounded-lg px-3 py-2.5 text-sm text-[#2f3437] focus:outline-none focus:ring-2 focus:ring-[#6ba3c7]/30"
          >
      ...
      {!selectedMemberId && (
        <div className="text-center py-12 text-[#2f3437]/40 text-sm">
      ...
      {selectedMemberId && !loading && !error && scripts.length === 0 && (
        <div className="bg-white border border-[#2f3437]/10 rounded-xl px-6 py-10 text-center">
          <DocumentTextIcon className="w-9 h-9 text-[#2f3437]/20 mx-auto mb-2" />
          <p className="text-sm font-semibold text-[#2f3437] mb-1">No scripts saved yet</p>
