  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-8">
      <div>
        ...
        <h1 className="text-2xl font-bold text-[#1e2a38]">AI Tools Usage</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-black text-[#3dc3ff]">{totalConversations}</div>
          <div className="text-xs text-[#1e2a38]/50 font-medium mt-1">Total Conversations</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-black text-[#1e2a38]">{uniqueMembers}</div>
          <div className="text-xs text-[#1e2a38]/50 font-medium mt-1">Active Members</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-black text-[#1e2a38]">
            {conversations.reduce((sum, c) => sum + c.downloadCount, 0)}
          </div>
          <div className="text-xs text-[#1e2a38]/50 font-medium mt-1">Total Downloads</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-black text-[#1e2a38]">
            {Object.keys(byTool).length}
          </div>
          <div className="text-xs text-[#1e2a38]/50 font-medium mt-1">Tools Used</div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-[#1e2a38]">Usage by Tool</h2>
        </div>
            ...
            return (
              <div key={toolType} className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#1e2a38]">{label}</p>
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-[#1e2a38]">
                    {data?.count ?? 0}
                  </div>
      ...
      {memberRows.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-[#1e2a38]">Member Activity</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-[#1e2a38]/50 uppercase tracking-wider">
                    Member
                  </th>
                  <th className="text-center px-3 py-2.5 text-xs font-semibold text-[#1e2a38]/50 uppercase tracking-wider">
                    Total
                  </th>
                  {Object.keys(TOOL_LABELS).map((t) => (
                    <th key={t} className="text-center px-3 py-2.5 text-xs font-semibold text-[#1e2a38]/50 uppercase tracking-wider">
                      {TOOL_ICONS[t]}
                    </th>
                  ))}
                  <th className="text-right px-4 py-2.5 text-xs font-semibold text-[#1e2a38]/50 uppercase tracking-wider">
                    Last Active
                  </th>
                </tr>
              </thead>
                  ...
                  <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <Link
                        ...
                        className="font-medium text-[#1e2a38] hover:text-[#3dc3ff] transition-colors"
                      >
                        {row.name}
                      </Link>
                      <p className="text-xs text-[#1e2a38]/40">{row.email}</p>
                    </td>
                    <td className="px-3 py-3 text-center">
                      <span className="inline-flex items-center justify-center w-7 h-7 bg-[#3dc3ff]/10 text-[#3dc3ff] text-xs font-bold rounded-full">
                        {row.total}
                      </span>
                    </td>
                    {Object.keys(TOOL_LABELS).map((t) => (
                      <td key={t} className="px-3 py-3 text-center text-xs text-[#1e2a38]/60">
                        {row.byTool[t] ?? "—"}
                      </td>
                    ))}
                    <td className="px-4 py-3 text-right text-xs text-[#1e2a38]/40">
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-[#1e2a38]">Recent Activity</h2>
        </div>
          ...
          {recentFeed.map((conv) => (
            <li key={conv.id} className="flex items-center justify-between px-4 py-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-[#1e2a38]/40">
                    <Link
                      ...
                      className="hover:text-[#3dc3ff] transition-colors"
                    >
              ...
              <div className="text-right shrink-0 ml-4">
                <p className="text-xs text-[#1e2a38]/40">{fmt(conv.createdAt)}</p>
                {conv.downloadCount > 0 && (
                  <p className="text-xs text-[#3dc3ff]">{conv.downloadCount} downloads</p>
                )}
              </div>
            </li>
          ))}
          {recentFeed.length === 0 && (
            <li className="px-4 py-6 text-center text-sm text-[#1e2a38]/40">
              No activity yet.
            </li>
          )}
        </ul>
