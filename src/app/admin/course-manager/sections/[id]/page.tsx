...
export default function EditSectionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (res.ok) {
      router.push("/admin/course-manager");
    } else {
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/course-manager" className="flex items-center gap-1.5 text-sm text-[#2f3437]/50 hover:text-[#2f3437] transition-colors">
          <ArrowLeftIcon className="w-4 h-4" />
          Course Manager
        </Link>
        <span className="text-[#2f3437]/30">/</span>
        <span className="text-sm text-[#2f3437] font-medium">Edit Section</span>
      </div>

      <h1 className="text-xl font-bold text-[#2f3437] mb-6">Edit Section</h1>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-5">
        ...
        <div>
          ...
          />
          <p className="text-xs text-[#2f3437]/40 mt-1">Used in URLs — letters, numbers, hyphens only.</p>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-[#2f3437] mb-1.5">Sort Order</label>
          <input
            type="number"
            value={form.sortOrder}
            onChange={(e) => setForm((f) => ({ ...f, sortOrder: parseInt(e.target.value) || 1 }))}
            className="w-24 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#6ba3c7] focus:border-transparent outline-none"
          />
        </div>
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href="/admin/course-manager" className="px-5 py-2.5 border border-gray-200 text-[#2f3437] text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
          ...
          {!confirmDelete ? (
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-[#e63946] font-medium">Are you sure?</span>
              <button
              ...
              <button onClick={() => { setConfirmDelete(false); setDeleteError(null); }} className="px-3 py-1.5 text-xs text-[#2f3437]/60 hover:text-[#2f3437] transition-colors">
                Cancel
              </button>
            </div>
          )}
