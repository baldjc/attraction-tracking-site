...
- **No Placeholder / Filler Numbers:** **FAIL** and quote the line if any quantitative claim is a malformed or filler stand-in instead of a clean traceable value — e.g. "the 0K range," "$500,000-to-the 600K range," "a meaningful amount," or a dangling value verb with no number ("days on market average sitting."). Every number must be a real figure (e.g. "$612,000," "49.4%," "3.2 months of inventory") or be cut entirely.
- **No Unsourced Factual Claims (specifics must trace to data or a Knowledge Base profile):** Grounding applies to specific factual claims, not just market numbers — **qualitative** specifics count too. **FAIL** and quote the line if the script asserts a specific, verifiable claim about the area that cannot trace to the member's data or Knowledge Base neighbourhood profile, including: a **build era / housing stock** claim ("built primarily in the 1990s," "developed in the mid-2010s," "single-story ranch styles," "newer construction with modern amenities"); a **demographic** claim, numeric OR worded ("median household income of $95,000," "median age of 34," "median income runs higher than the regional average," "skews older," "home to young families and first-time buyers"); a population count; a **dated event** ("the new school that opened in 2019," "built in the 1980s"); or a **named institution / amenity attribute** presented as fact (a specific school + rating, "HOA fees," "energy-efficient construction," "school ratings drew families"). **STAYS ALLOWED — do NOT flag:** the creator's interpretation of the data ("buyers are being methodical," "sellers are learning to price realistically," "the market rewards well-priced listings"), framework mechanics (MOI thresholds), and clearly-experiential, non-specific framing ("I've seen this area appeal to families," "a family-friendly area," "after years working this market"). When a qualitative specific IS sourced to the profile it must be cited in "## Sources"; invented specifics must be cut, kept general, or reframed as experience. Quote the claim and recommend either grounding it in a real source or generalising it.
- **No Source-of-Truth Disagreement (canonical = aggregate):** The member's Source-of-Truth aggregate is the canonical value the script must use. **FAIL** and quote the line if a script number contradicts its Source-of-Truth aggregate for the same metric and neighbourhood beyond simple rounding — for example citing 4.3 months of inventory when the aggregate is 3.8 (a per-fact value of 4.29 does NOT license writing 4.3 over the 3.8 aggregate). Flag the mismatch and tell the creator to use the aggregate value or drop the number.
- **One Canonical Variant Per Metric:** A metric can have several variants (e.g. months of inventory as a strict vs. an inclusive/CREB-aligned calculation; days on market as median vs. average). The script must cite ONE canonical variant consistently for a given neighbourhood. **FAIL** and quote both lines if the script (or its data summary / chat recap) cites two different values for the SAME metric and neighbourhood that are actually different variants — e.g. "6.71 months in Downtown" in one place and "8.8 months in Downtown" in another. Tell the creator to pick the canonical variant and use it everywhere.
- **Every Number Must Be Sourced (every family AND every shape):** Every specific quantitative claim about the member's market must trace to a fact AND appear in the "## Sources" footnote mapped to a fact id — regardless of family OR number shape. This covers the standard families (months of inventory, median/average price, price per square foot, sale-to-list ratio, days on market, failure rate, absorption) AND non-standard shapes that are easy to miss: comparison/temporal stats ("buyers are taking 40% longer to make an offer than early 2024"), "in [year]" figures, "X% above/below asking", and "X-Y%" ranges ("failure rates run 15-20%", "selling 5-10% below asking"). Unsourced sale-to-list "100% / 99% of asking" stated as DATA, and industry-norm numbers stated as fact about this market, are fabrications. **FAIL** and quote the number if any such figure is spoken in the body but missing from "## Sources" — treat it exactly like an unsourced months-of-inventory figure. Tell the creator to add the source bullet, or cut/generalise the number (no specific figure). **ALLOWED unsourced (do NOT flag):** framework constants / thresholds — the MOI bands ("anything below 2.5 months is a sellers market", "above 4.0 is a buyers market") and definitional statements ("100% of asking means full price"); and structural numbers — lead-magnet "1/3" tags, timestamps, title numbers (3/5/7/10), section counts. Qualitative framing with no specific figure is fine.
- **Credibility Cadence Must Be Stored, Never Guessed:** The Expertise Bridge may state a personal cadence ("our team helps a family move every X hours") ONLY when that figure is on the member's credentials profile. **FAIL** and quote the line if the script states ANY cadence the profile doesn't carry — a specific number ("every 53 hours") OR a vague frequency ("every few days," "every couple of days"). When nothing is on file, the bridge must be a non-frequency experience line ("after years of running this analysis…"). Tell the creator to use the real stored cadence or drop the frequency entirely.
- **No Self-Interruption Filler:** **FAIL** and quote the line if the script contains self-interrupting filler that breaks the authoritative voice — "wait a second, let me back up," "wait, let me back up," "let me back up," "hold on, let me back up," "actually, let me back up," "let me start over," "let me rephrase that," or similar mid-thought resets. These read as unscripted hesitation; recommend deleting them or rewriting the line to land cleanly the first time.
...
- **Placeholder / Filler Numbers:** [FAIL with the quoted line if any number is a malformed/filler stand-in ("the 0K range," "$500,000-to-the 600K," "a meaningful amount," dangling "average sitting.") instead of a clean traceable value.]
- **Unsourced Factual Claims:** [Scan for specific verifiable claims about the area — demographics (median income/age, population), dated events ("opened in 2019," "built in the 1980s"), named-institution attributes — that cannot trace to the member's data or a Knowledge Base neighbourhood profile. FAIL with the quoted claim and recommend grounding it in a real source or generalising it. Do NOT flag general/experience-based statements.]
- **Source-of-Truth Disagreement:** [FAIL with the quoted line if a number contradicts its Source-of-Truth aggregate for the same metric/neighbourhood beyond rounding (e.g. 4.3 months cited when the aggregate is 3.8). Tell the creator to use the aggregate value or drop the number.]
- **Self-Interruption Filler:** [FAIL with the quoted line if the script contains self-interrupting filler ("wait a second, let me back up," "let me back up," "actually, let me back up," "let me start over," and similar mid-thought resets). Recommend deleting it or rewriting the line to land cleanly.]
...
- **Revelation:** Hybrid Loops — Value Loop (What → Why → When → Proof Point → Meaning) for concept points, Data Tour Loop (Name It → Number It → Interpret It → Opinion It → Bridge It) for data points. Proof Density (client stories, data as proof, metaphors, personal experience — all valid). Data Specificity. NO placeholder/filler numbers — FAIL and quote the line on any malformed/filler stand-in ("the 0K range," "$500,000-to-the 600K," "a meaningful amount," dangling "average sitting.") instead of a clean traceable value. NO unsourced factual claims — FAIL and quote the line on any specific verifiable claim about the area (demographics like median income/age or population, dated events like "opened in 2019," named-institution attributes) that can't trace to the member's data or a Knowledge Base neighbourhood profile; general/experience statements are fine. NO Source-of-Truth disagreement — the aggregate is canonical; FAIL and quote the line on any number contradicting its Source-of-Truth aggregate for the same metric/neighbourhood beyond rounding (e.g. 4.3 months when the aggregate is 3.8). NO self-interruption filler — FAIL and quote the line on resets like "wait a second, let me back up," "let me back up," "actually, let me back up," "let me start over." Editorial Reactions after data points. Unique reframes. Grade 5 language. Curiosity Bridges (And/But/Therefore).
