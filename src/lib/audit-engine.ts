...
- **Every Number Must Be Sourced (all families, not just MOI + price):** Every market number the script speaks — months of inventory, median/average price, price per square foot, sale-to-list ratio, days on market, failure rate, absorption — must appear in the "## Sources" footnote mapped to a fact id. **FAIL** and quote the number if a real market figure (especially failure rates like "181%" / "72%", sale-to-list ratios, or days-on-market values) is spoken in the body but missing from "## Sources". Tell the creator to add the source bullet or cut the number — a clean draft sources EVERY number, not only inventory and price.
