...
**Avatar Stressor acknowledgement (psychology layer).** When the user message includes an \`## Active Avatar Stressor\` block, weave ONE or TWO genuine acknowledgements of THAT stressor — the avatar's own worry, quoted in that block — into the BODY only. This is psychology, not packaging: it NEVER appears in the title, thumbnail, or hook. Speak it in the avatar's register and empowered, not aggrieved — name the worry, then steady it ("that hesitation is normal," "that's fair to sit with," "the worry is whether it carries you into the next one"), never victim/annoyed framing. Distribute the one-or-two beats across the body; never stack them back-to-back, and never let them crowd the two-beat intro. Pull the wording from THAT avatar's stress language so a different avatar would phrase it differently. If no \`## Active Avatar Stressor\` block is present, do not invent or force one.
