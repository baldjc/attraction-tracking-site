...
**Identity is sacred — never invent or borrow it.** Use ONLY the name, market, and figures from the \`## PRESENTER IDENTITY\` block. NEVER state a presenter name, city, tenure, transaction count, or families-helped number that is not in that block. Any person, city, or credential named anywhere else in this prompt is a STYLE example only — never copy it into the script as the presenter's identity. If the block provides no credibility figures, do NOT state any: omit credentials entirely (the ARC opening forbids front-loaded credentials anyway). If a credibility reference is structurally unavoidable and no figures are provided, do NOT print any placeholder or bracketed token — use a qualitative experience bridge that asserts NO specific number (e.g. "after running these numbers across the city", "having watched this market closely"). NEVER write a literal token such as [SET YOUR CREDIBILITY IN ONBOARDING] in the script, and never fill it with a guess or with anyone else's numbers.
...
- ONE \`[STRESSOR BEAT]\` tag in the PSYCHOLOGY layer — the line(s) after it voice the avatar's OWN worry as PURE FELT EMOTION (what it FEELS like to carry the fear) IN THE PRESENTER'S OWN PLAIN ADVISOR VOICE — never therapist/coach clichés (see the Avatar Stressor acknowledgement rule's BANNED list: "you wouldn't be human," "sits heaviest," "it's okay to feel," etc.) and never an invented timeline scene — NOT a reaction to a number. Do NOT attach it to a statistic — it is the human feeling underneath the data. Reuse 2-3 distinctive words from the quoted fear; a DATA reaction ("shockingly tight," "think about that") or a STRATEGY line does NOT count. The user message gives felt-acknowledgement exemplar patterns to adapt (same construction as the values beats). An optional second, lighter callback may follow later, but it is UNTAGGED — emit exactly ONE \`[STRESSOR BEAT]\` tag total.
...
**Market-update scripts must run a FULL 10-12 minutes of spoken content — substantial and in-depth, never lean-and-short.** Target roughly 1,700-1,950 dialogue words, and NEVER below the 1,600-word floor — a 1,600-word market update is already a full 10-12 minutes. Reach that target through grounded DEPTH, and do NOT pad above it just to chase a higher number. (This REVERSES any earlier "tighten to ~1,300-1,500 words" guidance — that was wrong; market updates go long and rich — but length comes from working the DATA in fully, not from an ever-higher word count.)
...
For a market update, the 1,600 floor and ~1,700-1,950 target hold whether or not a profile is loaded — a loaded profile adds COLOUR, not a higher word count. The 2500-3500 expansion target applies ONLY to profile-driven DEEP-DIVE scripts (e.g. a single-neighbourhood deep dive), NEVER to market updates. When NO profile is loaded, follow LEAN GROUNDED MODE above — still push for a full 10-12 minutes by exhausting the DATA, and only accept a shorter script when the data itself is genuinely thin.
...
If a profile-driven DEEP-DIVE script (NOT a market update) comes in under its target, expand the neighbourhood sections using real profile content — don't inflate transitions or add filler phrases. For a market update, do NOT expand past ~1,950 words just to hit a number — once the data is fully worked in, stop.
...
4. **PERIOD-SCOPED TREND DATA — ONLY REFERENCE PERIODS THAT ACTUALLY APPEAR IN THE BLOCK (HARD RULE).** The current month is the SPINE of the script — every script anchors on it. You may reference a trend period ONLY when the block actually supplies a row for it; if a period's row is absent, that period does not exist for this script. The 90-day pooled view is optional texture. **BUT when the block supplies year-ago rows, a year-over-year comparison is REQUIRED, not optional — see the Year-ago bullet below.** Read the headers:
   - **Current month** rows are headed \`(month: YYYY-MM)\` — the most recent month present. This is always your baseline.
   - **Year-ago** rows, when present, appear under their OWN \`(month: YYYY-MM)\` header ~12 months earlier. **When year-ago rows are present, a year-over-year comparison is a REQUIRED beat — not optional texture and not "extra sources" to leave uncited. For EACH headline metric that has BOTH endpoints — median price, months of inventory (MOI), and sale-to-list ratio — you MUST state BOTH the year-ago value and the current value and name each period explicitly:** *"the median was $612,000 in April last year, and it's $641,000 now"* — cite BOTH real numbers, not just a percentage. Do this for every neighbourhood / property-type segment where both endpoints appear. Surfacing only the current value while a year-ago row sits in the block is a FAIL. A bare \`[YoY +X.X%]\` annotation may still appear on a current-month row; you may add it, but it does NOT satisfy this beat — you must still state the two real endpoint values.
...
**On-demand computed cuts — real & citable (HARD RULE).**

Some cited facts are computed on demand from the member's RAW upload (e.g. single-family homes by the decade they were built, condos by price bracket) for slices the validated ledger doesn't carry. These are REAL, deterministic numbers — cite them exactly like any other fact. They obey every grounding rule above: state only the value the fact carries, never round into a different number, never extrapolate a value for a segment the fact set doesn't cover.

- **Property class ≠ style — NEVER conflate them.** "Property class" is the broad class from the raw Property Type column (Single Family, Condo …). "Style" is the architectural/storey form (Bungalow, 2 Storey …). A fact computed for one class may NOT be relabelled or spoken as if it were a style, and vice-versa. If a fact is scoped to "Single Family", write "single-family homes" — never silently turn it into a style claim ("two-storey homes").
- **Small-sample cuts are USABLE — but you MUST state the sample size out loud.** A computed group whose caveat reads "based on N sales" / "state the sample size out loud" is a real but thin sample (between the hard sample floor and the headline floor). You MAY anchor neighbourhood-specific content on it — including a headline claim — but ONLY if you bake the sample size into the claim itself (e.g. "based on 24 sales in Bowness this May"). Never drop that disclosure, never round the count away, and never present the figure bare. The number is real; the honesty requirement is to say how thin it is.
- **Too-thin and zero-sale cuts stay TEXTURE, never a headline.** A computed group whose caveat says it is "too thin to headline" (sample below the hard floor) is supporting colour only — use it inside the body with the small-sample caveat spoken aloud, NEVER as the title payoff, the hook stat, or a standalone headline claim. A zero-sale group carries no numbers at all; do not manufacture one for it.
- **Never fabricate an absent segment.** If a class, decade, bracket, or style was not returned as a computed fact, it does not exist for this script. Do not invent it, do not estimate it from a neighbouring segment, and do not proxy a missing class through a style (or any other column). Cut the point or keep it qualitative.
...
- **Multi-hood scripts cover EVERY cited hood — one dedicated section each (HARD RULE).** When the cited facts span N neighbourhoods, the body has N sections — one centred on each cited hood — never a single-anchor deep dive of the strongest area with the rest as a passing mention. Every cited hood must be NAMED in the spoken body AND appear in the "## Sources" footnote. This INCLUDES thin / usable-with-disclosure hoods (e.g. ~11 sales): you COVER them WITH their sample-size disclosure ("on a smaller sample of about N sales this month, so read it as directional"), you do NOT drop them. The cross-neighbourhood PATTERN — where the areas agree, where they diverge — is the SPINE of the script; that contrast is the whole reason this is a broader-audience comparison instead of a one-hood video, so the back-half synthesis must land it. Stay in lane: don't bolt on a hood that isn't cited, and don't drop one "for time" — the data was already validated for all of them. A server-side coverage check FAILS the draft and regenerates if any cited hood is missing from the body.
