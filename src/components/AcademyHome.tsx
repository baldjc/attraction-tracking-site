...
function SectionCardSkeleton() {
  return (
    <div className="bg-white dark:bg-[#1a2433] border border-[#eaeaea] dark:border-white/10 rounded-xl p-5 animate-pulse">
      <div className="flex items-start gap-4">
        <div className="w-10 h-10 rounded-lg bg-[#eaeaea] dark:bg-white/10 shrink-0" />
        <div className="flex-1 min-w-0">
          <div className="h-4 bg-[#eaeaea] dark:bg-white/10 rounded w-3/4 mb-2" />
          <div className="h-3 bg-[#eaeaea] dark:bg-white/10 rounded w-1/3 mb-4" />
          <div className="h-1.5 bg-[#eaeaea] dark:bg-white/10 rounded-full mb-2" />
          <div className="h-3 bg-[#eaeaea] dark:bg-white/10 rounded w-1/4" />
        </div>
      </div>
    </div>
  );
...
export default function AcademyHome() {
  ...
  function HeroCard() {
    if (loading) {
      return (
        <div className="bg-gradient-to-r from-[#6ba3c7]/5 to-[#6ba3c7]/10 dark:from-[#6ba3c7]/10 dark:to-[#6ba3c7]/15 border border-[#6ba3c7]/20 dark:border-[#6ba3c7]/30 rounded-2xl p-6 animate-pulse">
          <div className="h-3 bg-[#6ba3c7]/20 rounded w-24 mb-3" />
          <div className="h-3 bg-[#6ba3c7]/20 rounded w-40 mb-2" />
          <div className="h-6 bg-[#6ba3c7]/20 rounded w-2/3 mb-5" />
          <div className="h-1.5 bg-[#6ba3c7]/20 rounded-full" />
        </div>
      );
    }
    ...
    if (allComplete) {
      return (
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-200 dark:border-green-700/40 rounded-2xl p-6">
          ...
          <p className="text-[#2f3437] dark:text-white font-semibold text-lg mb-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 text-sm font-medium text-[#2f3437]/60 dark:text-white/50 hover:underline"
            >
              Review Lessons
            </Link>
          </div>
    ...
    return (
      <Link
        ...
        className="block bg-gradient-to-r from-[#6ba3c7]/5 to-[#6ba3c7]/10 dark:from-[#6ba3c7]/10 dark:to-[#6ba3c7]/15 border border-[#6ba3c7]/20 dark:border-[#6ba3c7]/30 rounded-2xl p-6 hover:border-[#6ba3c7]/40 dark:hover:border-[#6ba3c7]/50 transition-colors group"
      >
        <div className="flex items-start gap-4">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 bg-[#6ba3c7]/15 dark:bg-[#6ba3c7]/20 rounded-full h-1.5 overflow-hidden">
                <div
                  className="h-full bg-[#6ba3c7] rounded-full transition-all"
                  style={{ width: `${progressPct}%` }}
                />
              </div>
              <span className="text-xs text-[#2f3437]/55 dark:text-white/40 shrink-0 tabular-nums">
                {continueTarget.sectionCompletedCount}/{continueTarget.sectionLessonCount} lessons
              </span>
            </div>
          </div>

          <div className="shrink-0 flex items-center justify-center w-10 h-10 rounded-xl bg-[#6ba3c7]/15 dark:bg-[#6ba3c7]/20 group-hover:bg-[#6ba3c7]/25 transition-colors mt-1">
            <ArrowRightIcon className="w-5 h-5 text-[#6ba3c7]" />
          </div>
        </div>
      </Link>
    );
        ...
        {orderedSections.map((sec, idx) => {
            ...
            return (
              <Link
                ...
                className="bg-white dark:bg-[#1a2433] border border-[#eaeaea] dark:border-white/10 rounded-xl p-5 hover:border-[#6ba3c7]/40 dark:hover:border-[#6ba3c7]/40 hover:shadow-sm transition-all group flex flex-col gap-4"
              >
                {/* Header row */}
                ...
                <div className="space-y-1.5">
                  <div className="bg-[#eaeaea] dark:bg-white/10 rounded-full h-1.5 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${
                        isComplete ? "bg-green-500" : "bg-[#6ba3c7]"
                      }`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-[#2f3437]/50 dark:text-white/40 tabular-nums">
                    ...
                    {isComplete ? (
                      ...
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-[#6ba3c7]/10 dark:bg-[#6ba3c7]/20 text-[#6ba3c7]">
                        In Progress
                      </span>
                    ) : (
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-[#eaeaea] dark:bg-white/10 text-[#2f3437]/50 dark:text-white/40">
                        Not Started
                      </span>
                    )}
                  </div>
  ...
  const colourMap: Record<string, string> = {
    ...
    blue: "bg-[#6ba3c7]/10 dark:bg-[#6ba3c7]/15 text-[#6ba3c7] border-[#6ba3c7]/20 dark:border-[#6ba3c7]/30",
    grey: "bg-[#eaeaea] dark:bg-white/10 text-[#2f3437]/60 dark:text-white/50 border-[#eaeaea] dark:border-white/10",
  };
      ...
      return (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="bg-white dark:bg-[#1a2433] border border-[#eaeaea] dark:border-white/10 rounded-xl p-4 animate-pulse"
            >
              <div className="w-8 h-8 rounded-lg bg-[#eaeaea] dark:bg-white/10 mb-3" />
              <div className="h-4 bg-[#eaeaea] dark:bg-white/10 rounded w-3/4 mb-2" />
              <div className="h-3 bg-[#eaeaea] dark:bg-white/10 rounded w-full" />
            </div>
          ))}
        </div>
      );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {resourceCards.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className="bg-white dark:bg-[#1a2433] border border-[#eaeaea] dark:border-white/10 rounded-xl p-4 hover:border-[#6ba3c7]/40 dark:hover:border-[#6ba3c7]/40 hover:shadow-sm transition-all group flex flex-col gap-3"
          >
            <div
            ...
            <div>
              <p className="font-semibold text-[#2f3437] dark:text-white text-sm leading-snug group-hover:text-[#6ba3c7] transition-colors">
                {card.label}
              </p>
              <p className="text-xs text-[#2f3437]/50 dark:text-white/40 mt-0.5 leading-relaxed">
                {card.description}
              </p>
            </div>
          </Link>
