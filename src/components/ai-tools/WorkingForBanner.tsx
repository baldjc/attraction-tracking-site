...
export default function WorkingForBanner() {
  ...
  return (
    <div className="mb-6 bg-[#3dc3ff]/8 border border-[#3dc3ff]/20 rounded-xl px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <UserCircleIcon className="w-4 h-4 text-[#1e2a38]/40 shrink-0" />
          <span className="text-xs text-[#1e2a38]/50 font-medium shrink-0">Working for:</span>
          {impersonate ? (
            <span className="text-xs font-semibold text-[#1e2a38] truncate">{impersonate.memberName}</span>
          ) : (
            <span className="text-xs text-[#1e2a38]/30 italic">No member selected — using your own data</span>
          )}
        </div>
        ...
        <div className="relative shrink-0" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-[#3dc3ff] border border-[#3dc3ff]/30 px-3 py-1.5 rounded-lg hover:bg-[#3dc3ff]/10 transition-colors whitespace-nowrap"
          >
            ...
            <div className="absolute right-0 top-full mt-1 w-72 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 overflow-hidden">
                  ...
                  filtered.map((m) => (
                    <li key={m.id}>
                      <button
                        ...
                        className={`w-full flex items-center gap-2 px-3 py-2.5 text-left hover:bg-[#3dc3ff]/5 transition-colors ${
                          impersonate?.memberId === m.id ? "bg-amber-50" : ""
                        }`}
                      >
                        <UserCircleIcon className="w-4 h-4 text-gray-400 shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-[#1e2a38] truncate">{m.fullName ?? m.email}</p>
              ...
              {impersonate && (
                <div className="p-2 border-t border-gray-100">
                  <button
                    onClick={clearMember}
                    className="w-full text-xs text-[#1e2a38]/50 hover:text-[#1e2a38] py-1.5 rounded-lg hover:bg-gray-50 transition-colors"
                  >
