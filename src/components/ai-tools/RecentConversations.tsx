    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden mb-6">
      ...
      >
        <div className="flex items-center gap-2">
          <ChatBubbleLeftRightIcon className="w-4 h-4 text-[var(--abv-text)]/40" />
          <span className="text-xs font-semibold text-[var(--abv-text)]/60 uppercase tracking-wider">
            {label}
          </span>
          {conversations.length > 0 && (
            <span className="text-xs bg-[var(--abv-ai-tools)]/10 text-[var(--abv-ai-tools)] font-semibold px-1.5 py-0.5 rounded-full">
      ...
      {open && (
        <div className="bg-white">
          {loading ? (
            <p className="px-4 py-3 text-xs text-[var(--abv-text)]/40 animate-pulse">Loading…</p>
          ) : conversations.length === 0 ? (
            <p className="px-4 py-3 text-xs text-[var(--abv-text)]/40">{emptyLabel}</p>
          ) : (
                ...
                <li key={conv.id}>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-[var(--abv-text)] truncate">{conv.title}</p>
                    ...
                    <div className="flex items-center gap-1 shrink-0 ml-2">
                      <button
                        ...
                        title="Download"
                        className="p-1.5 rounded-lg text-[var(--abv-text)]/30 hover:text-[var(--abv-ai-tools)] hover:bg-[var(--abv-ai-tools)]/10 transition-colors"
                      >
                      ...
                      <button
                        ...
                        title="Delete"
                        className="p-1.5 rounded-lg text-[var(--abv-text)]/30 hover:text-[var(--abv-crimson)] hover:bg-[var(--abv-crimson)]/10 transition-colors disabled:opacity-40"
                      >
